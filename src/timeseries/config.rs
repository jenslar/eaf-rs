...
use crate::{EafError, timeseries::TrackSource};
...
impl TimeSeriesConfig {
    pub fn new() -> Self {
        Self {
            date: UtcDateTime::now().to_string(),
            version: "1.0".to_string(),
            tracksources: Vec::new()
        }
    }

    /// Read time series configuration file.
    pub fn read(path: &Path) -> Result<Self, EafError> {
        Self::de(path)
    }
    ...
    fn de(path: &Path) -> Result<Self, EafError> {
        ...
        quick_xml::de::from_str::<Self>(&std::fs::read_to_string(path)?)
    ...
    pub fn add_source(&mut self, source: TrackSource) {
        self.tracksources.push(source);
    }

    pub fn add_sources(&mut self, sources: &[TrackSource]) {
        self.tracksources.extend_from_slice(sources);
    }

    pub fn with_sources(self, sources: &[TrackSource]) -> Self {
        Self {
            tracksources: sources.to_owned(),
            ..self
        }
    }

    /// Returns file paths to all linked timeseries CSV-files.
    pub fn source_paths(&self) -> Result<Vec<PathBuf>, EafError> {
        self.tracksources
            .iter()
            .map(|ts| ts.source_path())
            .collect()
    }

    pub fn sources(&self) -> impl Iterator<Item = &TrackSource> {
        self.tracksources.iter()
    }

    pub fn sources_len(&self) -> usize {
        self.tracksources.len()
    }

    pub fn tracknames(&self) -> impl Iterator<Item = &str> {
        self.tracksources.iter()
            .flat_map(|t| t.tracknames())
    }
}
